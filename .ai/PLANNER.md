...
bundles (fact 2; `26-09-03-00-changes`'s T1.1.1 added `_repo-sync-indicator.scss`, the 29th). `scripts/` holds
three small Node-only build/lint/coverage tools, each documented by its own header docstring — read
the docstring before the script body (`scripts/build-css.mjs:1-21`, `scripts/lint-tokens.mjs:1-29`,
`scripts/coverage-stories.mjs:1-50`). `test/` holds
...
| gate | command | result |
|---|---|---|
| token-lint ratchet | `node scripts/lint-tokens.mjs` | `lint:tokens — 59 violations in 18 files (baseline: 59)`, **exit 0** — unmoved by `26-09-16-01-workflow-execution-runner`'s T1.1.1/T1.1.2 (new `--w6w-step-color` is component-scoped, never registered in `_tokens.scss`; no new global token minted). `26-09-03-00-changes`'s T1.1.1 briefly regressed this (a hard-coded `6px` in `_repo-sync-indicator.scss:30` instead of `--w6w-sp-1-5`), undetected by that task's own contract/eval since neither ran this gate; caught and fixed at that project's own closeout (`a0027cd`) — **this gate is not covered by `tsc`/`biome`/`npm test`/`check:css` at all; run it explicitly, don't assume the other four imply it** |
| lint | `./node_modules/.bin/biome check .` | `Checked 152 files in 37ms. No fixes applied.` — 0 errors, measured on this project's T2.1.1 (unchanged by T2.1.1 itself: `biome.json`'s `files.include` is `src/**/*.ts(x)`, `vite.config.ts`, `.storybook/**/*.ts(x)` only — `scripts/` is never in scope, so the new `scripts/coverage-stories.mjs` adds 0). `122` → `152` (+30 new files) already happened before this task, at this project's own T1.1.1–T1.1.3: 28 new `*.stories.tsx` files (10 in T1.1.1, 11 in T1.1.2, 7 in T1.1.3) plus `.storybook/manager.ts` + `.storybook/fixtures.ts` (T1.1.3) — the only non-story files the include glob reaches. Prior: `Checked 122 files in 37ms. No fixes applied.` — 0 errors, measured on `26-09-19-01-python-runtime`'s closeout merge to `main`. `121` → `122` (+1 new file): T1.2.4 added `src/__tests__/ParamsForm.script-language.test.ts` (no new source file — `CodeEditor.tsx`, `ParamsForm.tsx` and `flow-types.ts` were extended in place: a `language` prop + JS/Python CodeMirror mode map on `CodeEditor`, the `effective()` sibling-value getter threaded into `ParamField`'s `code` branch, and a `language`-conditional default snippet). Prior: `Checked 121 files in 39ms. No fixes applied.` — 0 errors, measured on `26-09-16-01-workflow-execution-runner`'s closeout merge to `main`. `115` → `121` (+6 new files): T1.1.1 added `src/run-visuals.ts` + `src/__tests__/run-visuals.test.ts` + `src/__tests__/WorkflowFlowEditor.run-state-wiring.test.ts` (the run-status→visual pure module + its tests, plus AST-structural tests for `WorkflowFlowEditor.tsx` itself — no jsdom-mount precedent exists anywhere in this file for that component); T1.1.2 added `src/components/ExecutionLogPanel.tsx` + `src/components/StepStatusPill.tsx` + `src/components/__tests__/ExecutionLogPanel.test.ts` (both dispatched in parallel, disjoint footprints, confirmed zero file overlap by both evaluators). Prior: `Checked 115 files in 36ms. No fixes applied.` — 0 errors, measured on `26-09-15-00-updates`'s closeout merge to `main`. `111` → `115`: T2.1.1 added `src/app-pages.ts` + `src/__tests__/app-pages.test.ts` + `src/__tests__/AppPicker.paged.test.ts` + `src/__tests__/StepBuilderModal.ready-to-use.test.ts` (4 new files; `test/picker-paging/*` is outside biome's scanned scope here, same as `test/picker-layout/*` already was). Prior: `109` → `111`: T1.2.1 added `src/use-enter-submit.ts` + `src/__tests__/use-enter-submit.test.ts` (2 new files; `AddConnectionModal.tsx`/`AuthFieldsForm.tsx` were extended, not new). Prior: `107` → `109`: T1.1.1 added `YamlEditor.tsx` + `src/__tests__/YamlEditor.test.ts` (mirroring `JsonEditor.tsx`/`JsonEditor.copy.test.ts`, minus the lint/validity path — D-T0.0-4). Prior row: `105` → `106`: T1.1.1 (of `26-09-03-00-changes`, a different, same-named-item project) added `params-form-groups.test.ts`. `106` → `107`: T2.1.1 added `ActionTestForm.overrides.test.ts` |
| typecheck | `./node_modules/.bin/tsc -b --noEmit` | exit 0, no output |
| unit suite | `node --import ./src/test-jsx-loader.mjs --test src/__tests__/*.test.ts src/components/__tests__/*.test.ts` | **537 pass, 0 fail** (`tests 537 · pass 537 · fail 0`), measured on `26-09-19-01-python-runtime`'s closeout merge to `main`. `530` → `537` (+7, one new file): T1.2.4's `ParamsForm.script-language.test.ts` — the `language` select drives `CodeEditor`'s mode, `effective()`-threading reaches `ParamField`'s `code` branch, and the language-conditional default snippet survives at the `values` level, not just the rendered doc (round 2 moved the swap mechanism there after round 1 found the naive render-level fix left the doc showing stale text for ~200ms while `values.code` was already correct — `@uiw/react-codemirror` suppresses the inbound value-prop overwrite briefly after a keystroke, so a test asserting only `view.state.doc.toString()` can pass on a mutant that clobbers `values.code` unconditionally; FOLLOWUPS.md F-9 in that project records the one remaining gap: case (c) still asserts the doc, not `values.current.code`, so that specific mutant is caught by the shipped code but not by this row's own gate). Prior: **530 pass, 0 fail** (`tests 530 · pass 530 · fail 0`), measured on `26-09-17-01-updates`'s closeout merge to `main`. `526` → `530` (+4, no new file): `WorkflowFlowEditor.tsx` gained a new `onTestRun?: () => void` prop (fired at test-start click, threaded through `performRunStep` and `StepEditModal`'s footer Test button — mirroring `StepTestRunProps`'s existing optional-callback idiom) and `ExecutionLogPanel` gained `onDismiss?: () => void`, rendered as a header row built on the existing `IconButton` primitive, present in BOTH its populated and `steps.length === 0` empty-state branches (the latter is the exact state the reported bug's screenshot showed). One new case added to the existing `src/__tests__/WorkflowFlowEditor.test-tab.test.ts` (`onTestRun` spy fires on the Test-button click); three new cases added to the existing `src/components/__tests__/ExecutionLogPanel.test.ts` (dismiss button present+wired in both branches when `onDismiss` is supplied, absent in both when it is not). **This row's own prior baseline was stale**: it read `521` (`26-09-16-01-workflow-execution-runner`'s own number) but this project's pickup measured `526` on a clean `main` — some project in between added 5 cases without correcting this table (not investigated further, same drift shape this file already flags elsewhere). Prior (as last correctly attributed): **521 pass, 0 fail** (`tests 521 · pass 521 · fail 0`), measured on `26-09-16-01-workflow-execution-runner`'s closeout merge to `main`. `489` → `521` (+32): T1.1.1 added 23 (19 pure-function cases in `run-visuals.test.ts` + 4 AST-structural cases in `WorkflowFlowEditor.run-state-wiring.test.ts`); T1.1.2 added 9 in `ExecutionLogPanel.test.ts`. Both independently re-verified by their own evaluators (re-derived mutants in scratch extractions, not the developers' self-reports). Prior: **489 pass, 0 fail** (TAP: `tests 489 · pass 489 · fail 0`), measured on `26-09-15-00-updates`'s closeout merge to `main`. `448` → `489` (+41): T2.1.1's `AppPicker.paged.test.ts`/`StepBuilderModal.ready-to-use.test.ts`/`app-pages.test.ts` (new files) plus extensions to the existing `StepBuilderModal.{connection-only,homepage-tabs,template-node}.test.ts` fixtures (D-17/D-17a's genuinely-absent-optional-method correction) — not independently re-attributed case-by-case; the aggregate delta is measured directly. Prior: `437` → `448`: T1.2.1 added 11 cases in the new `src/__tests__/use-enter-submit.test.ts` (bare-Enter fire, textarea/contenteditable/modifier-key exclusions, `enabled:false` off-switch, `preventDefault` ordering — the full M1–M6 mutation table). Prior: `431` → `437`: T1.1.1 added `src/__tests__/YamlEditor.test.ts` (6 cases covering the mutation table's M1–M5 rows plus 2 copy-button smoke tests), mirroring `JsonEditor.copy.test.ts`'s rig. This row's prior `410` was already stale before an earlier task touched anything — re-measured on `main` @ `752b31e` (that task's own fork point, clean tree) at **413**, not `410`: some project between `26-09-03-00-changes`'s ship and that one added 3 tests without correcting this table (not investigated further). That task's own T1.1.1 then added 13: 9 in the new `params-form-groups.test.ts` (`GroupField`/`RepeatField` rendering) and 4 new cases in the existing `StepBuilderModal.required-gate.test.ts` (`requiredParamsFilled`'s new `group` branch), landing at `413` → `426`. **T2.1.1 then added 5 more** in the new `ActionTestForm.overrides.test.ts` (the Overrides region's mutation battery), landing at `426` → `431` |
| `check:css` | `node scripts/build-css.mjs --check` | both `src/styles.css` and `src/code.css` up to date |
| story coverage | `node scripts/coverage-stories.mjs` | `coverage:stories — 32 exported components, 32 with stories, 0 missing`, exit 0 — new in this project's T2.1.1 (zero-tolerance gate, no baseline file: enumerates every component `package.json`'s `exports` map reaches and fails if any has no co-located `*.stories.tsx` declaring `component: <Name>`) |
| `build-storybook` | `pnpm build-storybook` | `Storybook build completed successfully` (Vite build, `storybook-static/`, gitignored), exit 0 — re-run at this project's T2.1.1 pickup; `32/32` story files indexed (`storybook-static/index.json`), `117` named stories, `0` misses (`.ai/projects/artifacts/story-index-check.mjs`) |
...
| picker-paging (Docker/Chromium) | `bash test/picker-paging/run.sh` | **GREEN 11/11**, new in `26-09-15-00-updates` (T2.1.1: I-2 bounded/paged app selector for 500-2000 apps). Mounts the real `AppPicker`/`StepBuilderModal` against a 2,000-entry catalog fixture — bounded initial response, explicit server search/filter request, next-cursor pagination, query-change reset. Mirrors `picker-layout`'s own scratch-copy/esbuild/Playwright mechanics. |
...
verified: 2026-09-23 · against `proj/26-09-23-01-ui-storybook-and-public-docs-T2.1.1` (project
26-09-23-01-ui-storybook-and-public-docs, T2.1.1 — measured on its own task branch, forked from the
project's integration workspace after T1.1.1–T1.1.3 + T1.2.1 merged; not yet merged to `main`, so no
post-merge sha to cite here. *Where work happens* corrected to three `scripts/` tools
(`scripts/coverage-stories.mjs` added); gate table gained `story coverage` and `build-storybook` rows
(both new in this project); `lint` row's `122` → `152` (+30: the story sweep, T1.1.1–T1.1.3 — 28
`*.stories.tsx` files + `.storybook/manager.ts` + `.storybook/fixtures.ts`) had already happened
before T2.1.1 touched anything — `scripts/` sits outside `biome.json`'s `files.include`, so
`coverage-stories.mjs` itself adds 0; unit suite (`537`), `lint:tokens` (`59/18`) and `check:css` are
all unmoved by this task, re-measured directly rather than assumed; prior verification 2026-09-19 ·
`c308bd3`, 26-09-19-01-python-runtime closeout — gate table updated: unit suite 530→537 (+7, one new
file, `ParamsForm.script-language.test.ts`), biome 121→122 (+1, same file — no other new file;
`CodeEditor.tsx`/`ParamsForm.tsx`/`flow-types.ts` extended in place), lint:tokens/check:css untouched
by that diff; prior verification 2026-09-17 · `1a8a9be`, 26-09-17-01-updates closeout; before that
`4f285a0`, 26-09-16-01-workflow-execution-runner closeout; before that `2d4f404`, 26-09-11-00-fixes
closeout)
